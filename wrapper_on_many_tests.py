...
SPEED_PATH_F1 = './benchmarks_F1/speed/crypto_sign/xmss/ref'
SPEED_PATH_F2 = './benchmarks_F2/speed/crypto_sign/xmss/ref'
SPEED_PATH_F3 = './benchmarks_F3/speed/crypto_sign/xmss/ref'

HASH_PATH_F1 = './benchmarks_F1/hashing/crypto_sign/xmss/ref'
HASH_PATH_F2 = './benchmarks_F2/hashing/crypto_sign/xmss/ref'
HASH_PATH_F3 = './benchmarks_F3/hashing/crypto_sign/xmss/ref'

SIZE_PATH_F1 = './benchmarks_F1/size/crypto_sign/xmss/ref'
SIZE_PATH_F2 = './benchmarks_F2/size/crypto_sign/xmss/ref'
SIZE_PATH_F3 = './benchmarks_F3/size/crypto_sign/xmss/ref'

STACK_PATH_F1 = './benchmarks_F1/stack/crypto_sign/xmss/ref'
STACK_PATH_F2 = './benchmarks_F2/stack/crypto_sign/xmss/ref'
STACK_PATH_F3 = './benchmarks_F3/stack/crypto_sign/xmss/ref'

N = 300  # no. files per folder, easier to have it preset



def speed_converter():  # returning averages
    ...
    KP_list = np.zeros(N)
    SN_list = np.zeros(N)
    VR_cycles = np.zeros(N)
    VR_list = np.zeros(N)


    for SPEED_PATH in [SPEED_PATH_F1, SPEED_PATH_F2, SPEED_PATH_F3]:
        ...
        for i in np.arange(len(speed_data)):
            KP_list[i] += float(speed_data[i][speed_data[i].find('\n')+len('\n'):speed_data[i].rfind(sn)])/(24*np.power(10, 6)) /3
            SN_list[i] += float(speed_data[i][speed_data[i].find(sn)+len(sn):speed_data[i].rfind(vr)])/(24*np.power(10, 6))  /3
            VR_list[i] += float(speed_data[i][speed_data[i].find(vr)+len(vr):-1])/(24*np.power(10, 6)) /3
            VR_cycles[i] += int(speed_data[i][speed_data[i].find(vr)+len(vr):-1]) // 3  # divide by 3 to get averages

    return KP_list, SN_list, VR_list, VR_cycles


def min_speed_converter():  # returning averages

    sn = '\nsign cycles: \n'
    vr = '\nverify cycles: \n'

    KP_list = np.ones(N)*float('inf')
    SN_list = np.ones(N)*float('inf')
    VR_cycles = np.ones(N)*float('inf')
    VR_list = np.ones(N)*float('inf')
    ...
    for SPEED_PATH in [SPEED_PATH_F1, SPEED_PATH_F2, SPEED_PATH_F3]:
        ...
        speed_data = [g.read() for g in speed_data]
        for i in np.arange(len(speed_data)):
            KP_list[i] = np.minimum(float(speed_data[i][speed_data[i].find('\n')+len('\n'):speed_data[i].rfind(sn)])/(24*np.power(10, 6)), KP_list[i])
            SN_list[i] = np.minimum(float(speed_data[i][speed_data[i].find(sn)+len(sn):speed_data[i].rfind(vr)])/(24*np.power(10, 6)), SN_list[i])
            VR_list[i] = np.minimum(float(speed_data[i][speed_data[i].find(vr)+len(vr):-1])/(24*np.power(10, 6)), VR_list[i])
            VR_cycles[i] = np.minimum(int(speed_data[i][speed_data[i].find(vr)+len(vr):-1]), VR_cycles[i])
...
def hash_converter():  # returning averages
    ...
    keypair_cycles = np.zeros(N)
    keypair_hash_cycles = np.zeros(N)
    sign_cycles = np.zeros(N)
    sign_hash_cycles = np.zeros(N)
    verify_cycles = np.zeros(N)
    verify_hash_cycles = np.zeros(N)

    for HASH_PATH in [HASH_PATH_F1, HASH_PATH_F2, HASH_PATH_F3]:
        ...
        for i in np.arange(len(hash_data)):
            keypair_cycles[i] += float(hash_data[i][hash_data[i].find('\n')+len('\n'):hash_data[i].rfind(s1)])/(24*np.power(10, 6)) /3
            keypair_hash_cycles[i] += float(hash_data[i][hash_data[i].find(s1)+len(s1):hash_data[i].rfind(s2)])/(24*np.power(10, 6)) /3
            sign_cycles[i] += float(hash_data[i][hash_data[i].find(s2)+len(s2):hash_data[i].rfind(s3)])/(24*np.power(10, 6)) /3
            sign_hash_cycles[i] += float(hash_data[i][hash_data[i].find(s3)+len(s3):hash_data[i].rfind(s4)])/(24*np.power(10, 6)) /3
            verify_cycles[i] += float(hash_data[i][hash_data[i].find(s4)+len(s4):hash_data[i].rfind(s5)])/(24*np.power(10, 6)) /3
            verify_hash_cycles[i] += float(hash_data[i][hash_data[i].find(s5)+len(s5):-1])/(24*np.power(10, 6)) /3
...
def size_converter():
    ...
    for SIZE_PATH in [SIZE_PATH_F1, SIZE_PATH_F2, SIZE_PATH_F3]:
    ...
    return txt//3, dat//3, bss//3, total//3
...
def stack_converter():
    ...
    crypto_sign_keypair = np.zeros(N)
    crypto_sign = np.zeros(N)
    crypto_sign_open = np.zeros(N)

    for STACK_PATH in [STACK_PATH_F1, STACK_PATH_F2, STACK_PATH_F3]:
    ...
    return crypto_sign_keypair//3, crypto_sign//3, crypto_sign_open//3



KP, SN, VR, VRC = min_speed_converter()
keypair_c, keypair_hash_c, sign_c, sign_hash_c, verify_c, verify_hash_c = hash_converter()
KP_min, SN_min, VR_min, VRC_min = min_speed_converter()  # to use minimum for the plots
TXT, DAT, BSS, TOTAL = size_converter()
KP_stack, SN_stack, VR_stack = stack_converter()
VRC = VRC.astype(int)
VRC_min = VRC_min.astype(int)
...
print()
print("\\begin{table}[ht]\n\\caption{Average Time Performance of XMSS for different heights}")
...
ax1.plot(T, VR_min[12:108:12], marker='o', linewidth=0.5, label="BLOCK=0")
ax1.plot(T, VR_min[108:204:12], marker='o', linewidth=0.5, label="BLOCK=1")
ax1.plot(T, VR_min[204::12], marker='o', linewidth=0.5, label="BLOCK=2")
ax1.set_xlabel('$T$')
...
ax2.plot(T, VR_min[12+1:108:12], marker='o', linewidth=0.5, label="BLOCK=0")
ax2.plot(T, VR_min[108+1:204:12], marker='o', linewidth=0.5, label="BLOCK=1")
ax2.plot(T, VR_min[204+1::12], marker='o', linewidth=0.5, label="BLOCK=2")
ax2.set_xlabel('$T$')
...
ax3.plot(T, VR_min[12+2:108:12], marker='o', linewidth=0.5, label="BLOCK=0")
ax3.plot(T, VR_min[108+2:204:12], marker='o', linewidth=0.5, label="BLOCK=1")
ax3.plot(T, VR_min[204+2::12], marker='o', linewidth=0.5, label="BLOCK=2")
ax3.set_xlabel('$T$')
...
ax4.grid()
ax4.set_title('both')
ax4.plot(T, VR_min[12+3:108:12], marker='o', linewidth=0.5, label="BLOCK=0")
ax4.plot(T, VR_min[108+3:204:12], marker='o', linewidth=0.5, label="BLOCK=1")
ax4.plot(T, VR_min[204+3::12], marker='o', linewidth=0.5, label="BLOCK=2")
ax4.set_xlabel('$T$')
