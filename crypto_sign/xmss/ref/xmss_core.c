...
uint64_t besti = 0;
unsigned char msg_h_best1[32], msg_h_best2[32];
  ...
  const unsigned char *m, unsigned long long mlen)
{
  ...
  *(uint64_t*)(sm + params->sig_bytes + 32) = 0;
  ...
  sha256(msg_h, (sm + params->sig_bytes - 4 * params->n), mlen + 4 * params->n);
...

  {
    ...
    for (i = 1; i < ((uint64_t)1 << SHIFT); i++) {
      *(uint64_t*)(sm + params->sig_bytes + 32) = i;
      ...
      sha256(h2, (sm + params->sig_bytes - 4 * params->n), mlen + 4 * params->n);
#endif
                     ...
                     const unsigned char *m, unsigned long long mlen)
{
    ...
    *smlen += params->tree_height*params->n;
    ...
    if ((1 + idx_tree) * (1 << params->tree_height) + idx_leaf < (1ULL << params->tree_height)) {
    ...
    for (i = 0; i < params->d; i++) {
        ...
        if (! (((idx + 1) & ((1ULL << ((i+1)*params->tree_height)) - 1)) == 0)) {
            ...
            if ((1 + idx_tree) * (1 << params->tree_height) + idx_leaf < (1ULL << (params->tree_height - params->tree_height * i))) {
                if (i > 0 && updates > 0 && states[params->d + i].next_leaf < (1ULL << params->tree_height)) {
                    bds_state_update(params, &states[params->d + i], sk_seed, pub_seed, addr);
                    updates--;
                }
            }
        }
        else if (idx < (1ULL << params->tree_height) - 1) {
