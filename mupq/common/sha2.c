...
void sha256PreCompHash(uint8_t *out, const uint8_t *in, size_t inlen, const uint8_t *preCompState) {
#ifdef PROFILE_HASHING
    ...
    memcpy(&state, preCompState, sizeof(state));
    in += inlen;
    inlen &= 63;
    in -= inlen;
    ...
    if (inlen < 56) {
        ...
        crypto_hashblocks_sha256((unsigned char *)&state.ctx, padded, 64);
    } else {
        ...
        crypto_hashblocks_sha256((unsigned char *)&state.ctx, padded, 128);
    }
...
void sha256CreatePreCompState(uint8_t *out, const uint8_t *in, size_t inlen) {
#ifdef PROFILE_HASHING
    uint64_t t0 = hal_get_time();
#endif    
    ...
    (void) crypto_hashblocks_sha256((uint8_t *)&state, in, inlen);
