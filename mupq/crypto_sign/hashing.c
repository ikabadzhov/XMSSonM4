...
int main(void)
{
  ...
  unsigned char sm[SPPEED_TEST_ML+MUPQ_CRYPTO_BYTES];
  unsigned char m[SPPEED_TEST_ML];
  unsigned char m_out[SPPEED_TEST_ML + MUPQ_CRYPTO_BYTES];
  ...
  hash_cycles = 0;
  t0 = hal_get_time();
  MUPQ_crypto_sign(sm, &smlen, m, SPPEED_TEST_ML, sk);
  t1 = hal_get_time();
  ...
  hash_cycles = 0;
  t0 = hal_get_time();
  MUPQ_crypto_sign_open(m_out, &mlen, sm, smlen, pk);
  t1 = hal_get_time();
